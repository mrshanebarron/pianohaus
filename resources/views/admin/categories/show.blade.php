<x-layouts.admin title="{{ $category->name }}">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h2 class="text-xl font-display font-semibold text-ivory-100">{{ $category->name }}</h2>
        <a href="{{ route('admin.categories.edit', $category) }}" class="text-xs text-gold-400 hover:text-gold-300">Edit</a>
    </div>
    <p class="text-sm text-ivory-300 mb-6">{{ $category->description }}</p>
    <h3 class="text-sm font-semibold uppercase tracking-wider text-ebony-400 mb-3">Pianos ({{ $category->pianos->count() }})</h3>
    <div class="space-y-2">
        @foreach($category->pianos as $piano)
            <a href="{{ route('admin.pianos.show', $piano) }}" class="flex items-center justify-between p-3 bg-ebony-700/40 border border-ebony-600/30 rounded-lg hover:border-gold-500/30 transition">
                ...
                <span class="text-xs text-ebony-400">{{ $piano->brand->name ?? '' }} &middot; {{ ucfirst($piano->status) }}</span>
            </a>
        @endforeach
    </div>
</x-layouts.admin>
